...
Path.GIS <- "/Users/xiaomuliu/CrimeProject/SpatioTemporalModeling/RegionEval/Data/GISData/"
...
ord.R2K <- rep(NA,nrow(KDEgrd))
for (i in 1:nrow(KDEgrd)){
  ord.R2K[i] <- which(RegGrd[,1]==KDEgrd[i,1]&RegGrd[,2]==KDEgrd[i,2])
}

ord.R2K_full <- rep(NA,nrow(KDEgrd.full))
for (i in 1:nrow(KDEgrd.full)){
  ord.R2K_full[i] <- which(RegGrd.full[,1]==KDEgrd.full[i,1]&RegGrd.full[,2]==KDEgrd.full[i,2])
}

ord.K2R <- rep(NA,nrow(RegGrd))
for (i in 1:nrow(RegGrd)){
  ord.K2R[i] <- which(KDEgrd[,1]==RegGrd[i,1]&KDEgrd[,2]==RegGrd[i,2])
}

ord.K2R_full <- rep(NA,nrow(RegGrd.full))
for (i in 1:nrow(RegGrd.full)){
  ord.K2R_full[i] <- which(KDEgrd.full[,1]==RegGrd.full[i,1]&KDEgrd.full[,2]==RegGrd.full[i,2])
}